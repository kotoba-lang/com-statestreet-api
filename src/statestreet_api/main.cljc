(ns statestreet_api.main
  "Kotodama WASM entrypoint for the Statestreet_api clean-room actor (L4) — Clojure port.
  ...
  facts (`statestreet_api.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "statestreet_api")
(def tier "L4")
...
(def entity-specs
  [{:entity "Account"     :plural "accounts"     :id-prefix "statestr_acc"
    :fields [:holderId :type :balance :currency :status] :required [:type :balance]
    :coerce {:balance :float}                 :refs {}}
   {:entity "Loan"        :plural "loans"        :id-prefix "statestr_loa"
...
(defn emit-facts
  "EAVT facts for one record: {\"statestreet_api.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "statestreet_api-compat" :tier tier :entities entities} 200])
...
(defn start! [] :statestreet_api-compat/ready)
